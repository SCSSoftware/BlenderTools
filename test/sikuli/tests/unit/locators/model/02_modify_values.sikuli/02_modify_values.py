...
try:
    ...
    click(Pattern("model_name_empty.png").exact()); type("model_loc" + Key.ENTER); find(Pattern("3dview_model_name_changed.png").exact())
    click(Pattern("model_hookup_change.png").exact())
    paste("pedestrian_hookup mixer")
    click(Pattern("model_hookup_pedestrian_mixer_pick.png").exact())
    find(Pattern("model_hookup_pedestrian_mixer.png").exact())
except:
