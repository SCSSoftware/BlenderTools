...
try:
    wait(Pattern("startup_screen.png").exact(), 5); type(Key.ESC); hover(Pattern("startup_screen.png").exact())
    click("export_scene_button.png")
    wait(1.5)
    hover(Location(300, 400))  # move cursor to 3D view
    type(2 * Key.ESC)  # we expect warnings thus hide them
    ...
    wait(Pattern("startup_screen.png").exact())
except:
