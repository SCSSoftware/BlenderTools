...
def __get_config_ini_path():
    ...
    return os.path.join(splited_dir[0], "config.ini")
...
def save_screenshot(bundle_path, screen):
    ...
    """
    file = screen.capture(0, 0, 1050, 850)  # only capture blender area defined in get_startup_command
...
def delete_scs_tools_config():
    ...
    """
    blender_path = get_path_property("BlenderPath")
    for root, dirs, files in os.walk(os.path.dirname(blender_path)):
        if "io_scs_tools" in dirs:
            scs_tools_path = os.path.join(root, "io_scs_tools")
