...
_COLLECTION_NAME = _CS_consts.collection_name
...
def create_connection(loc0_obj, loc1_obj):
    ...
    :rtype: bool
    """
    return _core.create_connection(bpy.data.collections[_COLLECTION_NAME], loc0_obj, loc1_obj)
...
def delete_connection(loc0_name, loc1_name):
    ...
    :rtype: bool
    """
    return _core.delete_connection(bpy.data.collections[_COLLECTION_NAME], loc0_name, loc1_name)


def has_connection(loc0_obj, loc1_obj):
    result = _core.get_connection(bpy.data.collections[_COLLECTION_NAME], loc0_obj.name, loc1_obj.name)
...
def delete_locator(loc_name):
    ...
    :rtype: bool
    """
    return _core.delete_locator(bpy.data.collections[_COLLECTION_NAME], loc_name)
...
def rename_locator(old_name, new_name):
    ...
    :rtype: bool
    """
    return _core.rename_locator(bpy.data.collections[_COLLECTION_NAME], old_name, new_name)
...
def copy_check(old_objs, new_objs):
    ...
    """
    new_conns_count = _core.copy_connections(bpy.data.collections[_COLLECTION_NAME], old_objs, new_objs)
...
def get_neighbours(loc_obj):
    ...
    :rtype: list
    """
    conns_entries = _core.get_connections(bpy.data.collections[_COLLECTION_NAME], loc_obj.name)
...
def get_curves(np_locators):
    ...
    """
    connections = bpy.data.collections[_COLLECTION_NAME][_core.MAIN_DICT][_core.REFS][_core.CONNECTIONS][_core.ENTRIES]
    locators = bpy.data.collections[_COLLECTION_NAME][_core.MAIN_DICT][_core.REFS][_core.LOCATORS]
    ...
    conns_keys_dict = _core.gather_connections_upon_selected(bpy.data.collections[_COLLECTION_NAME], np_locs_names)
...
def force_recalculate(object_list):
    ...
    """
    _core.update_for_redraw(bpy.data.collections[_COLLECTION_NAME], object_list)


def invalidate():
    """Invalidates cached connectinos data, which will prevent connections to be drawn.
    """
...
def ready_for_draw():
    """Checks if connections cache is ready for draw.
    """
