...
class _Tokenizer():
    def __init__(self, data_input, include_path):
    ...
    def parse_token(self):
        ...
        while self.current_line < len(self.input):
            ...
            if match:
                file_name = self.include_path + match.group(1)
                file = open(file_name, 'r')
                included_lines = file.readlines()
                file.close()

                new_array = self.input[:self.current_line - 1]
...
def parse_file(filepath, print_info=False):
    ...
    include_path = os.path.split(filepath)[0] + os.sep
    # print('include_path: %r' % str(include_path))
    if not os.path.isdir(include_path):
        include_path = "/home/milos/Documents/SCS/trucks/trunk/eurotrucks2/root/base/def/world/"

    tokenizer = _Tokenizer(lines, include_path)
