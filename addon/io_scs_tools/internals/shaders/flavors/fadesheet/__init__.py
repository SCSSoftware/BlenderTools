...
from io_scs_tools.internals.shaders.flavors.fadesheet import fadesheet_compute_ng

FLAVOR_ID = "fadesheet"
FADESHEET_COMPUTE_NODE = "FadesheetCompute"
...
def __create_node__(node_tree):
    ...
    fadesheet_compute_n.name = fadesheet_compute_n.label = FADESHEET_COMPUTE_NODE
    fadesheet_compute_n.node_tree = fadesheet_compute_ng.get_node_group()


def init(node_tree, location, uv_output, base_texture_input, over_texture_input, base_mix_input, base_amix_input):
    ...
    :param over_texture_input: input socket of the over texture
    :type over_texture_input: bpy.types.NodeSocket
    :param base_mix_input: input socket of the base color mix
    :type base_mix_input: bpy.types.NodeSocket
    :param base_amix_input: input socket of the base alpha mix
    :type base_amix_input: bpy.types.NodeSocket
    """

    if FADESHEET_COMPUTE_NODE not in node_tree.nodes:
        __create_node__(node_tree)

        node_tree.nodes[FADESHEET_COMPUTE_NODE].location = location

    # links creation
    nodes = node_tree.nodes

    node_tree.links.new(nodes[FADESHEET_COMPUTE_NODE].inputs['UV'], uv_output)
    node_tree.links.new(base_texture_input, nodes[FADESHEET_COMPUTE_NODE].outputs['UV0'])
    node_tree.links.new(over_texture_input, nodes[FADESHEET_COMPUTE_NODE].outputs['UV1'])
    node_tree.links.new(base_mix_input, nodes[FADESHEET_COMPUTE_NODE].outputs['FrameBlend'])
    node_tree.links.new(base_amix_input, nodes[FADESHEET_COMPUTE_NODE].outputs['FrameBlend'])
...
def delete(node_tree):
    ...
    if FADESHEET_COMPUTE_NODE in node_tree.nodes:
        node_tree.nodes.remove(node_tree.nodes[FADESHEET_COMPUTE_NODE])
...
def get_node(node_tree):
    ...
    """
    return node_tree.nodes[FADESHEET_COMPUTE_NODE] if FADESHEET_COMPUTE_NODE in node_tree.nodes else None
