...
class SkinStream:
    ...
    class Entry:
        ...
        def __init__(self, piece_index, vertex_index, vertex_pos, bone_weights):
            ...
            self._bone_weights = OrderedDict()
            self._clones = OrderedDict()

            self._position = vertex_pos
            ...
            if weight_sum > 1:
                for bone_indx in bone_weights.keys():
                    self._bone_weights[bone_indx] = bone_weights[bone_indx] / weight_sum
            else:
                for bone_indx in bone_weights.keys():
                    self._bone_weights[bone_indx] = bone_weights[bone_indx]
        ...
        def add_clone(self, piece_index, vertex_index):
            ...
            if clone_hash not in self._clones:
                self._clones[clone_hash] = (piece_index, vertex_index)
                return True
        ...
        def get_original_piece_info(self):
            ...
            :rtype: tuple
            """
            return self._clones[list(self._clones.keys())[0]]
        ...
        def get_hash(self):
            ...
            key = str(self._position)

            for bone_index in sorted(self._bone_weights.keys()):
                key += str(bone_index) + str(int(self._bone_weights[bone_index] * perc))
        ...
        def get_section_repr(self):
            ...
            weights = []
            for bone_indx in sorted(self._bone_weights.keys()):
                weights.append((bone_indx, self._bone_weights[bone_indx]))

            clones = []
            for clone_hash in sorted(self._clones.keys()):
                clones.append(self._clones[clone_hash])

            return self._position, weights, clones

    _format = ""  # defined by type of tag
    _tag = Types.POSITION
    _item_count = 0
    _total_weight_count = 0
    _total_clone_count = 0
    ...
    def __init__(self, stream_type):
        ...
        self._total_weight_count = 0
        self._total_clone_count = 0
    ...
    def add_entry(self, skin_entry):
        ...
        if entry_hash not in self._data:  # create new entry
            ...
            self._total_weight_count += skin_entry.get_weight_count()
            self._total_clone_count += 1
        else:
    ...
    def get_as_section(self):
        ...
        self._item_count = len(self._data)

        section = _SectionData("SkinStream")

        section.props.append(("Format", self._format))
        section.props.append(("Tag", self._tag))
        section.props.append(("ItemCount", self._item_count))
        section.props.append(("TotalWeightCount", self._total_weight_count))
        section.props.append(("TotalCloneCount", self._total_clone_count))
